...
type DeckType = "adventure" | "party" | "horror" | "cyberpunk";

const ADVENTURE_DECK: TextCard[] = [
...
const DECKS: Record<DeckType, { label: string; cards: TextCard[] }> = {
  adventure: { label: "Adventure", cards: ADVENTURE_DECK },
  party: { label: "Party", cards: PARTY_DECK },
  horror: { label: "Horror", cards: HORROR_DECK },
  cyberpunk: { label: "Cyberpunk", cards: CYBERPUNK_DECK },
};

export default function CardsPage() {
  const [activeDeck, setActiveDeck] = useState<DeckType>("adventure");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-1 mb-6">
        {(Object.keys(DECKS) as DeckType[]).map((key) => (
          <button
            key={key}
            className="btn"
            style={{
              background: activeDeck === key ? "var(--accent)" : "transparent",
              color: activeDeck === key ? "white" : "var(--text-secondary)",
              border: activeDeck === key ? "none" : "1px solid var(--border)",
            }}
      ...
      <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>
        {deck.cards.length} cards in the {deck.label} deck
      </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {deck.cards.map((card) => (
          ...
          >
            <p className="text-sm font-medium leading-snug">{card.text}</p>
            <p
